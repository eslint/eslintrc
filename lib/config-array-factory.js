 ...
 * @property {string} [resolveSharedConfigsRelativeTo] A path to the directory that shared configs should be resolved from. Defaults to `cwd`.
 ...
 * @property {string | undefined} resolveSharedConfigsRelativeTo An absolute path the the directory that shared config files should be resolved from.
 * @property {Map<string,Rule>} builtInRules The rules that are built in to ESLint.
 * @property {Object} [resolver=ModuleResolver] The module resolver object.
 * @property {string} eslintAllPath The path to the definitions for eslint:all.
 * @property {Function} getEslintAllConfig Returns the config data for eslint:all.
 ...
 * @property {string} sharedConfigBasePath The base path to resolve shared configurations to extend.
 ...
 * @param {string} [resolveSharedConfigsRelativeTo] The resolveSharedConfigsRelativeTo config option
 * @returns {Error} The error object to throw
 * @private
 */
function configInvalidError(configName, importerName, messageTemplate, resolveSharedConfigsRelativeTo) {
    const messageData = { configName, importerName };

    if (resolveSharedConfigsRelativeTo) {
        messageData.resolveSharedConfigsRelativeTo = resolveSharedConfigsRelativeTo;
    }
...
function createContext(
    { cwd, resolvePluginsRelativeTo, resolveSharedConfigsRelativeTo },
    providedType,
    providedName,
    providedFilePath,
    ...
    const pluginBasePath =
        ...
        (filePath && path.dirname(filePath)) ||
        cwd;
    const sharedConfigBasePath =
        resolveSharedConfigsRelativeTo ||
        (filePath && path.dirname(filePath)) ||
        cwd;
    const type = providedType || "config";

    return { filePath, matchBasePath, name, pluginBasePath, sharedConfigBasePath, type };
}
...
class ConfigArrayFactory {
    ...
    constructor({
        ...
        internalSlotsMap.set(this, {
            ...
            resolveSharedConfigsRelativeTo:
                resolveSharedConfigsRelativeTo &&
                path.resolve(cwd, resolveSharedConfigsRelativeTo),
            builtInRules,
            resolver,
            eslintAllPath,
            getEslintAllConfig,
    ...
    *_normalizeObjectConfigDataBody(
    ...
    _loadExtendedShareableConfig(extendName, ctx) {
        const { resolver } = internalSlotsMap.get(this);
        const relativeTo = ctx.filePath || path.join(ctx.sharedConfigBasePath, "__placeholder__.js");
        let request;
