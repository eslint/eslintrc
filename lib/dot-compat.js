...
function translateESLintRC(eslintrcConfig, { resolveConfigRelativeTo, resolvePluginsRelativeTo }) {
    ...
    if (eslintrcConfig.criteria) {
        ...
        const { includes, excludes } = eslintrcConfig.criteria.patterns[0];

        if (includes) {
            dotConfig.files = [includes.pattern];
        }
        ...
        if (excludes) {
            ...
            */
            if (!includes) {
                dotConfig.files = [excludes.pattern.map(pattern => `!${pattern}`)];
            } else {
                dotConfig.ignores = excludes.pattern;
            }
...
console.dir(eslintrcArray[1].criteria);
        const dotArray = [];
